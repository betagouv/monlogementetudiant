...
import { createAcademy, createAccommodation, createExternalSource, createFavoriteAccommodation, createOwner } from './fixtures/factories'
...
describe('accommodations.listFavorites', () => {
  it('requires authentication', async () => {
    await expect(caller.accommodations.listFavorites()).rejects.toThrow('UNAUTHORIZED')
  })
  ...
  it('returns favorites of the current user only', async () => {
    ...
    const result = await authenticatedCaller.accommodations.listFavorites()
...
describe('accommodations.addFavorite', () => {
  it('requires authentication', async () => {
    await expect(caller.accommodations.addFavorite({ accommodationSlug: 'test' })).rejects.toThrow('UNAUTHORIZED')
  })
  ...
  it('adds a favorite', async () => {
    ...
    const result = await authenticatedCaller.accommodations.addFavorite({ accommodationSlug: 'add-fav-test' })
    expect(result.userId).toBe('test-user-id')

    const favorites = await authenticatedCaller.accommodations.listFavorites()
  ...
  it('is idempotent (no error if already favorite)', async () => {
    ...
    await authenticatedCaller.accommodations.addFavorite({ accommodationSlug: 'idempotent-fav' })
    await authenticatedCaller.accommodations.addFavorite({ accommodationSlug: 'idempotent-fav' })
...
describe('accommodations.removeFavorite', () => {
  it('requires authentication', async () => {
    await expect(caller.accommodations.removeFavorite({ slug: 'test' })).rejects.toThrow('UNAUTHORIZED')
  })
  ...
  it('removes a favorite', async () => {
    ...
    await authenticatedCaller.accommodations.removeFavorite({ slug: 'remove-fav-test' })
  ...
  it('does not affect other users favorites', async () => {
    ...
    await authenticatedCaller.accommodations.removeFavorite({ slug: 'other-user-fav' })

    const myFavorites = await authenticatedCaller.accommodations.listFavorites()
    expect(myFavorites).toHaveLength(0)

    const otherFavorites = await authenticatedCaller2.accommodations.listFavorites()
