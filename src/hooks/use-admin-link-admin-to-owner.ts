...
export const useAdminLinkAdminToOwner = () => {
  const queryClient = useQueryClient()
  const trpc = useTRPC()
  const trpcClient = useTRPCClient()

  return useMutation({
    mutationFn: (data: { userId: string; ownerId: number }) => trpcClient.admin.users.linkAdminToOwner.mutate(data),
    onSuccess: async (_, variables) => {
      ...
      createToast({ priority: 'success', message: 'Administrateur lié au gestionnaire' })
    },
    onError: (error) => {
