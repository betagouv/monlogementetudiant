...
export const useAccomodations = ({ initialData }: UseAccomodationsOptions = {}) => {
  ...
  const { accessible, bbox, colocation, disponible, page, prix } = queryStates
  const enabled = !!bbox || !!accessible || !!page || !!colocation || !!prix || !!disponible
  return useQuery<TGetAccomodationsResponse>({
    enabled,
    initialData: enabled ? undefined : initialData,
    queryFn: () => fetchAccomodations(bbox, page, accessible, colocation, disponible, prix),
    queryKey: ['accomodations', { accessible, bbox, colocation, page, disponible, prix }],
  })
}
