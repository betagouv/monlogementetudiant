    ...
    <div className={clsx(styles.card, 'fr-mb-3w')}>
      ...
      <div>
        {isLoading ? (
          ...
          <p className="fr-text--sm fr-text-mention--grey fr-p-3w">{"Vous n'êtes rattaché à aucun bailleur"}</p>
        ) : (
  ...
  return (
    <div className={styles.linkedOwnerItem}>
      {owner.imageBase64 && !imgError ? (
        <div className={styles.linkedOwnerAvatar} style={{ background: '#fff', overflow: 'hidden' }}>
...
function LinkSelfToOwnerDialog({ userId }: { userId: string }) {
  ...
  return (
    <linkSelfToOwnerModal.Component title="Se lier à un bailleur" size="large">
      <Input
              ...
              >
                <div
                  style={{
                    ...
                    background: getColorForName(o.name),
                    color: 'white',
                    display: 'flex',
                    alignItems: 'center',
