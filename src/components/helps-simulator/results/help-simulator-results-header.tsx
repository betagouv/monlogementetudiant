...
export const HelpSimulatorHeaderResults: FC = () => {
  ...
  if (currentStep !== 4 || !results || !formData) {
    return null
  }
    ...
    <div className="boxShadow fr-py-3w fr-px-8w">
      ...
      <div className="fr-flex fr-justify-content-space-between fr-align-items-center fr-mb-3w">
        <div className="fr-flex fr-flex-gap-2v">
          <span className="fr-text--bold fr-text--sm fr-mb-0">Plafond de loyer retenu</span>
          <Badge severity="new" noIcon>
            {results.zoneConfig.rentCap} € / mois
          </Badge>
        </div>
        <div className="fr-flex fr-flex-gap-2v">
      ...
      {results.rentExceedsCap && (
        <p className="fr-text--sm fr-mb-3w fr-text-mention--grey">
          Le loyer ({formData.monthlyRent}€) dépasse le plafond de la zone. Les calculs APL/ALS sont basés sur un loyer de{' '}
          {results.zoneConfig.rentCap} €.
        </p>
      )}
